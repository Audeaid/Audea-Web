import { IGetAllContent } from './getAllContent'

export const convertArrayToCSV = (array: IGetAllContent[] | null | undefined) => {
    ...
    keys
      .map((key) => {
        ...
        if (key === 'gptGenerated') {
          ...
          const contentArray = gptGeneratedArray.map((item: { content: string }) => item.content)
