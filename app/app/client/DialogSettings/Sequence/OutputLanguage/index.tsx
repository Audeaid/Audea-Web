...
export function OutputLanguage({ value, setValue, setValueDb, saved, setSaved }: Props) {
  ...
  return (
    <section className='flex flex-col gap-8'>
      <p>
        Choose from a diverse range of languages to ensure your transformed content resonates effortlessly with you.
      </p>

      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button variant='outline' role='combobox' aria-expanded={open} className='w-[250px] justify-between'>
            {val ? outputLanguageList.find((language) => language.value === val)?.label : 'Select language...'}
              ...
              {outputLanguageList.map((language) => (
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    ...
                    const db = outputLanguageList.find(({ value }) => currentValue === value)?.db as string
                ...
                >
                  <Check className={cn('mr-2 h-4 w-4', val === language.value ? 'opacity-100' : 'opacity-0')} />
                  {language.label}
                </CommandItem>
              ))}
            </CommandGroup>
