...
export const BulletPointsWithSummary = ({ content, dir }: Props) => {
  const summary: any[] = []

  const additionalInfo: any[] = []

  const bulletPointsToIterate: any[] = []
    ...
    <article className='flex flex-col gap-8' dir={dir}>
      <section className='flex flex-col gap-2'>
        {summary.map(({ type, content }, index) => {
          if (type === 'heading_1') {
            return (
              <h2 key={`${index}-summary`} className='sm:text-3xl text-2xl font-bold'>
                {content}
              </h2>
            )
      ...
      <section>
        <h2 className='sm:text-3xl text-2xl font-bold my-6'>{additionalInfo[0].content}</h2>

        <section className='flex flex-col gap-6'>
          {bulletPoints.map(({ content, children }, index) => {
            return (
              <section key={`${index}-bulletPoints`} className='flex flex-col gap-2'>
