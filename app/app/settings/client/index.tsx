...
import { IContentSettings } from '../graphql'
...
export default function Client({ contentSettings, token }: Props) {
  return (
    <motion.section
    ...
    >
      ...
      <section className='flex flex-col gap-10'>
        <OutputLanguage token={token} initialValue={contentSettings.outputLanguage} />

        <Separator />

        <WritingStyle token={token} initialValue={contentSettings.writingStyle} />

        <Separator />

        <SelectPrompt token={token} initialValue={contentSettings.typeOfPromptId} />
      </section>
