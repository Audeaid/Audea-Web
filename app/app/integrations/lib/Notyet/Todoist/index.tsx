...
export default function Todoist({ token, initialState }: { token: string; initialState: boolean }) {
      ...
      <CardHeader>
        <CardTitle className={cn('flex items-center justify-between')}>
          <div className='flex items-center gap-4'>
            <Image src={TodoistImage} quality={100} draggable={false} height={40} width={40} alt='Todoist logo' />
            Todoist
          </div>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                await userRequestedIntegration(token, 'TODOIST')
                setClick(true)
              } catch (error) {
                setClick(false)
