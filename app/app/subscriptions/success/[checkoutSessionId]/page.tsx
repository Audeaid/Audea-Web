...
import { createPaidObject, getPaidObject, purchasedSubscription } from './graphql'
...
export default async function Page({ params, searchParams }: Props) {
  ...
  try {
    ...
    const { data }: { data: Stripe.Response<Stripe.Checkout.Session> } = await axios.post(
      generateUrl('/api/stripe/retrieve-session').href,
      {
          ...
          <section className='min-w-screen min-h-screen bg-background text-foreground fixed inset-0 z-[99999999999] flex items-center justify-center select-none'>
            <section className='flex flex-col items-center justify-center text-center max-w-[300px] gap-4 mx-4'>
              ...
              <p className='text-sm font-light'>
                An error occurred while trying to resolve your payment. Most likely that your payment has gone through.
                Please contact support at <a href='mailto:support@audea.id'>support@audea.id</a> and we will resolve
                this issue ASAP.
              </p>
          ...
          return (
            <section className='min-w-screen min-h-screen bg-background text-foreground fixed inset-0 z-[99999999999] flex items-center justify-center select-none'>
              <section className='flex flex-col items-center justify-center text-center max-w-[300px] gap-4 mx-4'>
                ...
                <p className='text-sm font-light'>
                  This link has expired. This means that your payment has already been processed and paid.
                </p>
          ...
          return (
            <section className='min-w-screen min-h-screen bg-background text-foreground fixed inset-0 z-[99999999999] flex items-center justify-center select-none'>
              <section className='flex flex-col items-center justify-center text-center max-w-[300px] gap-4 mx-4'>
                ...
                <p className='text-sm font-light'>
                  An error occurred while trying to resolve your payment. Most likely that your payment has gone
                  through. Please contact support at <a href='mailto:support@audea.id'>support@audea.id</a> and we will
            ...
            <section className='flex flex-col items-center justify-center text-center max-w-[300px] gap-4 mx-4'>
              ...
              {response.type === 'MONTHLY' ||
                (response.type === 'YEARLY' && (
                  <p className='text-sm font-light'>
                    Your new subscriptions will end on {endDate.toLocaleTimeString()} {endDate.toLocaleDateString()}{' '}
                    local time.
                  </p>
                ))}
